...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.channel2 = torch.nn.ModuleList([torch.nn.Linear(self.channel_patch2*4, self.channel_patch2)
                            for i in range(self.layers)])
        self.c_bn = torch.nn.ModuleList([torch.nn.BatchNorm2d(self.seq_len)
                            for i in range(self.layers)])
        self.c_bn2 = torch.nn.ModuleList([torch.nn.BatchNorm2d(self.seq_len)
                            for i in range(self.layers)])
        ...
        self.pred_linear = torch.nn.ModuleList([torch.nn.Linear(self.seq_len, self.pred_len)for i in range(self.layers)])
    ...
    def encoder(self, x_enc, x_mark_enc):
        ...
        for i in range(self.layers):
            ...
            trend1 = t1 + self.dropout(self.t_pred_local2[i](self.dropout(self.gelu(self.t_pred_local[i](trend1.permute(0,1,3,2))))).permute(0,1,3,2))
            t1 = trend1 
            trend1 = self.t_bn2[i](trend1.permute(0,3,1,2)).permute(0,2,3,1)
            trend1 = t1 + self.dropout(self.t_pred_global2[i](self.dropout(self.gelu(self.t_pred_global[i](trend1.permute(0,3,2,1))))).permute(0,3,2,1))
            trend1 = trend1.reshape(batch, self.seq_len, channel)[:,:,:self.enc_in]
            ...
            season =  s1 + self.dropout(self.s_pred_local2[i](self.dropout(self.gelu(self.s_pred_local[i](season.permute(0,1,3,2))))).permute(0,1,3,2))
            s1 = season 
            season =  self.s_bn2[i](season.permute(0,3,1,2)).permute(0,2,3,1)
            season =  s1 +  self.dropout(self.s_pred_global2[i](self.dropout(self.gelu(self.s_pred_global[i](season.permute(0,3,2,1))))).permute(0,3,2,1))
