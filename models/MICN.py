...
class Model(nn.Module):
    ...
    def __init__(self, configs, conv_kernel=[12, 16]):
        ...
        if self.channel_function == 'RNN':
            ...
            self.pro = nn.Sequential( 
                torch.nn.Linear(self.seq_len*2, self.seq_len),
                nn.SiLU(),
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.channel_function == 'RNN':
            ...
            h0 = torch.randn(2, B, self.seq_len, device=x_enc.device)
            c0 = torch.randn(2, B, self.seq_len, device=x_enc.device)
            x_enc = x_enc + torch.mul(self.linear1(x_enc), self.pro(self.lstm(self.norm(x_enc), (h0,c0))[0]))
            x_enc = x_enc.permute(0,2,1)
