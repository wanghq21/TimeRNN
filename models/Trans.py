...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.emb_s = torch.nn.ModuleList([torch.nn.Linear(self.patch[i], self.d_model)
                            for i in range(self.layers)])
        self.transformer_t = torch.nn.ModuleList([EncoderLayer(
                    AttentionLayer(
                        FullAttention(False, configs.factor, attention_dropout=self.rate,
        ...
        self.transformer_s = torch.nn.ModuleList([EncoderLayer(
                    AttentionLayer(
                        FullAttention(False, configs.factor, attention_dropout=self.rate,
                                      output_attention=configs.output_attention), self.d_model, configs.n_heads),
                    self.d_model,
        ...
        self.prediction_t = torch.nn.ModuleList([torch.nn.Linear(self.d_model, self.patch[i])
                            for i in range(self.layers)])
        self.prediction_s = torch.nn.ModuleList([torch.nn.Linear(self.d_model, self.patch[i])
                            for i in range(self.layers)])
        ...
        # channel
        # self.emb_c = torch.nn.ModuleList([torch.nn.Linear(self.channel_patch2, self.d_model)
        #                     for i in range(self.layers)])
        # self.transformer_c = torch.nn.ModuleList([EncoderLayer(
        #             AttentionLayer(
        ...
        # self.pos_c = torch.nn.ModuleList([PositionalEmbedding(self.d_model) for i in range(self.layers)])
        # self.prediction_c = torch.nn.ModuleList([torch.nn.Linear(self.d_model, self.channel_patch2)
        #                     for i in range(self.layers)])
        # self.c_bn = torch.nn.ModuleList([torch.nn.BatchNorm2d(self.seq_len) for i in range(self.layers)])
    ...
    def encoder(self, x_enc, x_mark_enc):
        ...
        for i in range(self.layers):
            ...
            trend1 = self.transformer_t[i](trend1)[0]
            trend1 = t1 +  self.prediction_t[i](trend1).reshape(batch, channel, self.n_patch[i], self.patch[i])
            ...
            season = season.permute(0,3,1,2)
            s1 = season
            season = self.s_bn[i](season)
            season = season.reshape(batch*channel, self.n_patch[i], self.patch[i])
            season = self.emb_s[i](season) +self.pos_s[i](season)
            season = self.transformer_s[i](season)[0]
            season = s1 +  self.prediction_s[i](season).reshape(batch, channel, self.n_patch[i], self.patch[i])
            ...
            # trend1 = trend1.reshape(batch, self.seq_len, self.channel_patch, self.channel_patch2)
            # c1 = trend1
            # trend1 = self.c_bn[i](trend1)
            # trend1 = trend1.reshape(batch*seq, self.channel_patch, self.channel_patch2)
            # trend1 = self.emb_c[i](trend1)  
            # trend1 = self.transformer_c[i](trend1)[0]
            # trend1 = c1 + self.prediction_c[i](trend1).reshape(batch, self.seq_len, self.channel_patch, self.channel_patch2)
            # trend1 = trend1.reshape(batch, self.seq_len, -1)[:,:,:self.enc_in]
