...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.cnn_t  = torch.nn.ModuleList([torch.nn.Conv2d(self.enc_in, self.enc_in,kernel_size=(self.kernel[i],3),padding=((self.kernel[i]-1)//2,1), groups=self.enc_in) for i in range(self.layers)])
        self.cnn_t2  = torch.nn.ModuleList([torch.nn.Conv2d(self.enc_in, self.enc_in,kernel_size=(self.kernel[i],3),padding=((self.kernel[i]-1)//2,1), groups=self.enc_in) for i in range(self.layers)])
        self.cnn_t3  = torch.nn.ModuleList([torch.nn.Conv2d(self.enc_in, self.enc_in,kernel_size=(self.kernel[i],3),padding=((self.kernel[i]-1)//2,1), groups=self.enc_in) for i in range(self.layers)])
        self.cnn_s  = torch.nn.ModuleList([torch.nn.Conv2d(self.enc_in, self.enc_in,kernel_size=(self.kernel[i],3),padding=((self.kernel[i]-1)//2,1), groups=self.enc_in) for i in range(self.layers)])
        self.cnn_s2  = torch.nn.ModuleList([torch.nn.Conv2d(self.enc_in, self.enc_in,kernel_size=(self.kernel[i],3),padding=((self.kernel[i]-1)//2,1), groups=self.enc_in) for i in range(self.layers)])
        self.cnn_s3  = torch.nn.ModuleList([torch.nn.Conv2d(self.enc_in, self.enc_in,kernel_size=(self.kernel[i],3),padding=((self.kernel[i]-1)//2,1), groups=self.enc_in) for i in range(self.layers)])
        ...
        self.t_bn3 = torch.nn.ModuleList([torch.nn.BatchNorm2d(self.enc_in) for i in range(self.layers)])
        self.s_bn3 = torch.nn.ModuleList([torch.nn.BatchNorm2d(self.enc_in) for i in range(self.layers)])
    ...
    def encoder(self, x_enc, x_mark_enc):
        ...
        for i in range(self.layers):
            ...
            trend1  = t1 + self.dropout(self.gelu(self.cnn_t[i](trend1)))
            t1 = trend1
            trend1 = self.t_bn2[i](trend1)          
            trend1  = t1 + self.dropout(self.gelu(self.cnn_t2[i](trend1)))
            t1 = trend1
            trend1 = self.t_bn3[i](trend1)
            trend1  = t1 + self.dropout(self.gelu(self.cnn_t3[i](trend1)))
            ...
            season  = s1 + self.dropout(self.gelu(self.cnn_s[i](season)))
            s1 = season
            season = self.s_bn2[i](season)          
            season  = s1 + self.dropout(self.gelu(self.cnn_s2[i](season)))
            s1 = season
            season = self.s_bn3[i](season)
            season  = s1 + self.dropout(self.gelu(self.cnn_s3[i](season)))
        ...
        # print(torch.sum(torch.stack(loss, dim=-1)))
