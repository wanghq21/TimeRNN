...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        print(df_raw.shape)
        a = df_raw.iloc[1:, 1:].to_numpy()
        corr_matrix = np.corrcoef(a.T)
        corr_matrix = np.abs(corr_matrix)
        triu_corr_matrix = np.triu(corr_matrix, k=1)  # 只保留上三角
        mean_corr = np.nanmean(triu_corr_matrix)  # 计算上三角部分的平均值
        print("绝对平均相关系数:", mean_corr)
        corr_values = triu_corr_matrix[~np.isnan(triu_corr_matrix)]  # 获取非 NaN 值
        corr_variance = np.var(corr_values)
        print("绝对相关性强度的方差:", corr_variance)
